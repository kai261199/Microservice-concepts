...
export function MainPage() {
  ...
  const pollOrderStatus = useCallback((orderId: string) => {
    ...
    pollingRef.current = setInterval(async () => {
      ...
      if (attempts > 30) { clearInterval(pollingRef.current!); setPollingStatus(null); addLog('?? Timeout'); }
    }, 2000);
  }, [addLog]);
    ...
    <>
      ...
      {error && (
        <div style={{
          ...
          <AlertTriangle size={16} /> {error} - Ch?y docker-compose + 3 service, ho?c dùng Simulate.
        </div>
      )}
      ...
      <div style={{ padding:'20px 32px 0' }}>
        {steps.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontSize:16, fontWeight:700, color: t.accent, marginBottom:4 }}>Ch?n scenario ? trên</div>
            <div style={{ fontSize:13, color: t.textSecondary }}>Swim-lane graph hi?n message flow gi?a các service</div>
          </div>
        ) : (
          <div style={{
            ...
            <div style={{ animation:'fadeIn 0.3s ease' }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ color: isDark ? '#ff79c6' : '#ec4899' }}>?( ? )?</span>
                  <span style={{ margin: '0 12px', color: isDark ? '#8be9fd' : '#06b6d4' }}>?(????)?</span>
                  <span style={{ color: isDark ? '#50fa7b' : '#10b981' }}>?( ? )?</span>
                  <span style={{ margin: '0 12px', color: isDark ? '#f1fa8c' : '#f59e0b' }}>~(???~)</span>
                  <span style={{ color: isDark ? '#bd93f9' : '#8b5cf6' }}>(????)?*:???</span>
                  <span style={{ margin: '0 12px', color: isDark ? '#ff5555' : '#ef4444' }}>( ??? )?</span>
                  <span style={{ color: isDark ? '#ffb86c' : '#f97316' }}>?( ???)?</span>
                  <span style={{ margin: '0 12px', color: isDark ? '#f8f8f2' : '#64748b' }}>?(???)?</span>
